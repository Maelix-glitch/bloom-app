...
describe("sanitizeElements", () => {
  ...
  it("drops unknown stickers without a source URL", () => {
    expect(sanitizeElements([{ kind: "sticker", stickerId: "nope.missing" }])).toEqual([]);
  });

  it("keeps GIPHY stickers that carry a https source", () => {
    const [el] = sanitizeElements([
      {
        kind: "sticker",
        stickerId: "giphy:abc",
        src: "https://media.giphy.com/media/sticker.webp",
        still: "https://media.giphy.com/media/sticker_s.webp",
        width: 220,
        height: 220,
      },
    ]);
    ...
    if (el?.kind === "sticker") {
      expect(el.src).toBe("https://media.giphy.com/media/sticker.webp");
      expect(el.stickerId).toBe("giphy:abc");
    }
  });
