 ...
 * audio. GIFs search GIPHY only — no uploads, no local motion pack.
 */
...
const GIF_CHIPS = ["Happy", "Love", "Wow", "Dance", "Good morning", "Celebrate"];

export function CatalogSearchTray({
  title,
  subtitle,
  placeholder,
  searchLabel,
  ...
  unconfigured: string;
  onPick: (asset: GifAsset) => void;
  onClose: () => void;
}) {
  const [query, setQuery] = useState("");
  const [items, setItems] = useState<GifAsset[]>([]);
  const [loading, setLoading] = useState(false);
  const debounced = useDebounced(query);
  const heading = useMemo(() => (debounced.trim() ? "Results" : "Trending"), [debounced]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const run = debounced.trim() ? provider.search(debounced, 24) : provider.trending(24);
    void run.then((g) => alive && setItems(g)).finally(() => alive && setLoading(false));
    return () => {
      alive = false;
    };
  }, [debounced, provider]);
  ...
  return (
    <StorySheet title={title} subtitle={subtitle} onClose={onClose}>
      <div className="flex flex-col gap-3 pb-2">
        <label className="flex items-center gap-2.5 rounded-full border border-border bg-surface/60 px-4 py-2.5 transition-colors focus-within:border-border-strong">
          <Search className="size-4 shrink-0 text-faint" aria-hidden />
          <input
        ...
        {chips && chips.length > 0 ? (
          <div className="flex gap-1.5 overflow-x-auto pb-0.5" role="group" aria-label="Browse">
            {chips.map((chip) => (
              <button
                key={chip}
                type="button"
        ...
        {!provider.configured ? (
          ...
          <p className="py-8 text-center text-[13px] text-faint">
            {debounced.trim() ? `No ${title.toLowerCase()} match that.` : "Nothing here right now."}
          </p>
        ) : (
          <>
            ...
            <div className="grid grid-cols-3 gap-2">
              {items.map((item) => (
                <button
                  ...
                  className="overflow-hidden rounded-xl border border-transparent bg-surface-2/40 transition-all hover:border-border active:scale-95"
                >
                  <img
                    src={item.still}
                    alt=""
            ...
            {provider.attribution ? (
              <p className="mono pt-1 text-center text-[10px] uppercase tracking-[0.1em] text-faint">
                {provider.attribution}
              </p>
            ) : null}
          </>
...
export function GifTray({
  ...
  onPick: (gif: GifAsset) => void;
  onClose: () => void;
}) {
  return (
    <CatalogSearchTray
      title="GIFs"
      subtitle="A little motion, when words won't do."
      placeholder="Search GIFs…"
      searchLabel="Search GIFs"
      addLabel={(name) => `Add GIF: ${name}`}
      chips={GIF_CHIPS}
      provider={gifProvider}
      fit="cover"
      unconfigured="GIF catalog isn't connected."
      onPick={onPick}
      onClose={onClose}
    />
  );
