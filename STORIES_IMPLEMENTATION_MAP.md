...
| Brief step                  | Status                                                                                                                                                                                                                                |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **31 Viewers**              | **Absent.** `StoryViewRecord` is a _type declaration only_ — `grep` finds it in `src/lib/stories/types.ts` and nowhere else. No service function records or reads views, and no UI lists them. Needs backend work, not just a screen. |
| **16 Multi-slide stories**  | **Absent.** `Story` is a single composition — there is no `slides[]`, no `StorySlide` type, no slide timeline, no per-slide progress. This is the one structural gap, and it is large.                                                |
