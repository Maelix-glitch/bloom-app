# ============================================================================
#  BLOOM COACH FIX - one script, no downloads.
#  Run from your bloom-app folder (where supabase\ and src\ live).
#  It: writes the new edge function with failover, removes the model picker,
#  fixes the model to APInex (Gemini 3.8 Flash), deploys, commits and pushes.
# ============================================================================

Write-Host "==> Writing supabase\functions\coach\index.ts (failover chain)" -ForegroundColor Cyan
@'
/**
...
'@ | Set-Content -Path "supabase\functions\coach\index.ts" -Encoding UTF8 -NoNewline

Write-Host "==> Removing the model picker from the Coach page" -ForegroundColor Cyan
$coachPage = "src\components\coach\CoachPage.tsx"
$c = Get-Content $coachPage -Raw
$c = $c -replace '(?m)^import \{ ProviderPicker \} from "@/components/coach/ProviderPicker";\r?\n', ''
$c = $c -replace '(?m)^\s*<ProviderPicker lastSource=\{lastSource\} />\r?\n', ''
$c = $c -replace '(?m)^\s*/\*[^*]*picker[^*]*\*/\r?\n\s*const \[lastSource, setLastSource\][^\r\n]*\r?\n', ''
$c = $c -replace '(?m)^\s*const \[lastSource, setLastSource\] = useState<"edge" \| "local" \| undefined>\(undefined\);\r?\n', ''
$c = $c -replace '(?m)^\s*setLastSource\(response\.source\);\r?\n', ''
Set-Content -Path $coachPage -Value $c -Encoding UTF8 -NoNewline

Write-Host "==> Sending the fixed model from the hook" -ForegroundColor Cyan
$hook = "src\hooks\useCoachSystem.ts"
$h = Get-Content $hook -Raw
$h = $h -replace 'import \{ activeProvider \} from "@/lib/coach/providers";', 'import { COACH_PROVIDER } from "@/lib/coach/providers";'
$h = $h -replace 'provider: activeProvider\(\)\.id,', 'provider: COACH_PROVIDER,'
Set-Content -Path $hook -Value $h -Encoding UTF8 -NoNewline

Write-Host "==> Pinning the one model in providers.ts" -ForegroundColor Cyan
$prov = "src\lib\coach\providers.ts"
$p = Get-Content $prov -Raw
if ($p -notmatch 'COACH_PROVIDER') {
  $insert = 'export const DEFAULT_PROVIDER = "bloom";' + "`r`n`r`n" +
            '/* The one model the coach always uses. Fixed on purpose - there is no picker. */' + "`r`n" +
            'export const COACH_PROVIDER = "apinex";'
  $p = $p -replace 'export const DEFAULT_PROVIDER = "bloom";', $insert
}
Set-Content -Path $prov -Value $p -Encoding UTF8 -NoNewline

Write-Host "==> Deploying the coach function" -ForegroundColor Cyan
supabase functions deploy coach

Write-Host "==> Committing and pushing to GitHub" -ForegroundColor Cyan
git pull origin main
git add supabase/functions/coach/index.ts src/lib/coach/providers.ts src/hooks/useCoachSystem.ts src/components/coach/CoachPage.tsx
git commit -m "coach: fixed model (APInex) with automatic failover to teamo/groq/gemini; remove picker"
git push origin main

Write-Host ""
Write-Host "DONE." -ForegroundColor Green
Write-Host "If you have Groq/Gemini keys, add them to the coach function so the"
Write-Host "failover chain can use them (copy keys fresh from their sites first):"
Write-Host "  supabase secrets set GROQ_API_KEY=..."
Write-Host "  supabase secrets set GEMINI_API_KEY=..."
Write-Host "Test: open the app -> Coach. No model dropdown; it answers via APInex,"
Write-Host "and hops to DeepSeek/Groq/Gemini automatically if APInex fails."